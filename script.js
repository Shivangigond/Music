...
function popup(elemIndex) {
  ...
  // Update title, image, and audio source
  popupTitle.textContent = `${songs[elemIndex].title} by ${songs[elemIndex].artist}`;
  songImage.src = songs[elemIndex].artwork;
  myaudio.src = songs[elemIndex].url;

  // Dynamically create previous and next buttons
  controls.innerHTML = `
